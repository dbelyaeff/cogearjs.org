...
export default class Navbuttons
	...
	build: ->
		...
		if prev
			...
			prevLink.innerHTML = "<span class=\"icon is-small\"><i class=\"fas fa-arrow-left\"></i></span><span>#{prev.innerText}</span>"
		else
		...
		if next
			...
			nextLink.innerHTML = "<span>#{next.innerText}</span><span class=\"icon is-small\"><i class=\"fas fa-arrow-right\"></i></span>"
		else
